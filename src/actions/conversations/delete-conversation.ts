...
import { conversations, pronunciationsAssessment } from '@/lib/db/drizzle/schema'
...
export async function deleteConversation(convesationId: string): Promise<void> {
  return (await withAuth(
    async () => {
      ...
      await db.delete(pronunciationsAssessment).where(eq(pronunciationsAssessment.conversationId, convesationId))
    }
  ))()
}
